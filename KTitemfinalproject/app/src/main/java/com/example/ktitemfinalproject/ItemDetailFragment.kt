...
import com.example.ktitemfinalproject.databinding.FragmentItemDetailBinding
...
class ItemDetailFragment: Fragment() {
    ...
    override fun onCreateView(
        ...
        if (savedInstanceState != null) {
            itemDetailConnect(savedInstanceState.getInt("bm_no",1))
        }

        val commentButton = binding.commentButton
        commentButton.setOnClickListener{
            val comment : EditText = binding.commentEditText
            //단 댓글이 없을 시
    ...
    private fun initItemCommentRecyclerView(){
        ...
        //우리는 데이터를 담을때 linearlayout에 담을 것이기때문에 LinearLayoutManager를 넣어준다.
        binding.itemCommentsRecyclerView.layoutManager = LinearLayoutManager(requireContext())
        //데이터플 ActivityAllListItem의 id에 담아준다
    ...
    override fun onPrepareOptionsMenu(menu: Menu){
        ...
        val item = menu.findItem(R.menu.item_detail_menu)
    ...
    private fun itemDeleteConnect(br_sel_buy : String){
        itemService.getItemDeleteByName(item_no,br_sel_buy).enqueue(object: Callback<ResponseBody> {
            override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                ...
                Toast.makeText(requireContext(),"상품 삭제가 완료되었습니다.", Toast.LENGTH_SHORT).show()
    ...
    private fun goToMain(){
        ...
        fragmentManager?.beginTransaction()?.remove(this@ItemDetailFragment)?.commit()
        ...
        itemService.getCommentInsertByName(comment,0,0,item_no)
            .enqueue(object: Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    ...
                    Toast.makeText(requireContext(),"댓글이 입력되었습니다.", Toast.LENGTH_SHORT).show()
                }
